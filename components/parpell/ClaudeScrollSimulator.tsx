  ...
  return (
    ...
    >
      ...
      <div className="rounded-3xl bg-[#120610]/95 border-2 border-[#9E5C6A]/45 shadow-[0_25px_70px_rgba(0,0,0,0.85),0_0_35px_rgba(158,92,106,0.18)] overflow-hidden backdrop-blur-2xl text-left font-mono">
        ...
        <div className="px-5 sm:px-6 py-3.5 flex items-center justify-between border-b border-white/[0.08] bg-[#180A15]/90">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-zinc-400 font-mono ml-2.5 font-semibold flex items-center gap-1.5">
              <Bot className="w-3.5 h-3.5 text-[#C27A8A]" />
              <span>motor-ia / respuesta en tiempo real</span>
            </span>
          </div>

          <div className="flex items-center gap-2 text-[10px] text-[#C27A8A] font-bold">
            <span className="w-2 h-2 rounded-full bg-[#9E5C6A] animate-pulse" />
            <span>{isDone ? "RECOMENDACIÓN GENERADA" : "PROCESANDO RESPUESTA"}</span>
          </div>
        </div>

        {/* Window Content Body */}
        <div className="p-5 sm:p-7 space-y-5 bg-gradient-to-b from-[#120610] to-[#0A0308]">
          
          {/* User Prompt Query Box */}
          <div className="p-4 sm:p-5 rounded-2xl bg-white/[0.03] border border-[#9E5C6A]/30 space-y-2.5">
            <div className="flex flex-col sm:flex-row sm:items-baseline gap-2 text-sm sm:text-base">
              <span className="text-[#C27A8A] font-black tracking-wide text-xs uppercase font-mono px-2 py-0.5 rounded bg-[#9E5C6A]/20 border border-[#9E5C6A]/40 shrink-0 self-start">
                &gt; PROMPT:
              </span>
            ...
            <div className="flex items-center gap-2 text-xs text-zinc-400 font-mono pt-1.5 border-t border-white/[0.06]">
              <MapPin className="w-3.5 h-3.5 text-[#C27A8A]" />
              <span>Zona objetivo: <strong className="text-zinc-200">Torrelodones, Madrid</strong> · Filtro: Máxima Autoridad Local</span>
            </div>
          </div>
